...
import re
from constants import centene_wellcare_payment
import boto3
import io
import pandas as pd
...
def extract_html_content(site):
    ...
    temp = {'state' : site.split("/")[-5],
            'site_name':"https://www.centene.com/products-and-services/browse-by-state/" + site.split("/")[-5] + ".html",
...
def extract_pdf_links(centene_wellcare_payment):
    l1 = []
    for site in centene_wellcare_payment:
...
def save_policy_files_to_s3(centene_wellcare_payment):
    client_name = 'Centene'
    folder_name = 'Wellcare_Payment_Policies'
    bucket_name = 'zigna-nsa-payer-data'
    current_datetime = datetime.now()
    present_date = current_datetime.date()
    date = present_date

    downloadable_links = extract_pdf_links(centene_wellcare_payment)
